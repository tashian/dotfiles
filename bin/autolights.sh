...
log stream --predicate 'subsystem == "com.apple.UVCExtension" and composedMessage contains "Post PowerLog"' | while read line; do
  ...
  if echo "$line" | grep -q "= On"; then
    ...
    if i_am_at_home; then 
		#Light 1 - set to on
		curl --location --request PUT "http://${LIGHT_ADDR}:9123/elgato/lights" --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":40,"temperature":162,"on":1}],"numberOfLights":1}'
	fi
  ...
  if echo "$line" | grep -q "= Off"; then
    ...
    if i_am_at_home; then	  
		#Light 1 - set to off
		curl --location --request PUT "http://${LIGHT_ADDR}:9123/elgato/lights" --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":40,"temperature":162,"on":0}],"numberOfLights":2}'
	fi
